...
bracesArgParser :: String -> [Brackets]
...
data Group    = Group Brackets [Item] deriving Show
type Item     = Either String Group

-- Parser:

items :: [Brackets] -> Parsec String [Item]
items bs = many (item bs)

item :: [Brackets] -> Parsec String Item
item bs = choice (map (fmap Right . mkParser bs) bs) <|> fmap Left (nonBracket bs)

mkParser :: [Brackets] -> (String, String) -> Parsec String Group
mkParser bs p@(l,r) = do
  is <- between (string l) (string r) (items bs)
  return $ Group p is

nonBracket :: [Brackets] -> Parsec String String
